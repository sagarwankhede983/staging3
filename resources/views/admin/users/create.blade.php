...
<html lang="en">
    ...
    <div class="card-body">
        <form name="myForm" action="/addUserData" method="POST" enctype="multipart/form-data" >
            @csrf
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name">{{ trans('global.user.fields.name') }}*</label>
                <input  name="name" id="name_error_id" value="<?php if($namef!=""){echo $namef;}?>" class="form-control" required>
            ...
            <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                ...
                <input type="email" name="email" value="<?php if($emailf!=""){echo $emailf;} ?> " class="form-control" required>
                <p class="helper-block">
    ...
    <Select class="form-control" name="role" id="dynamic_select">
        ...
        $i=0;
            foreach ($roles as $key => $value) {
                $db_role=$roles[$i]['TITLE'];

        ?>
            <option>{{$db_role}}</option>
        <?php
            $i=$i+1;
        }
        ?>
