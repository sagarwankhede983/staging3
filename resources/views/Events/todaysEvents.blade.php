                                  ...
                                  <div class="card-header">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped table-hover datatable">
                                            ...
                                            <?php
                                            $i=0;
                                            foreach ($event_data_ar as $id => $eventcount) {
                                                $event_id=$event_data_ar[$i]['event_id'];
                                                $cat_event_type=$event_data_ar[$i]['cat_event_type'];
                                                $room=$event_data_ar[$i]['room'];
                                                $date_value=$event_data_ar[$i]['date_value'];
                                                $start_time=$event_data_ar[$i]['start_time'];
                                                $end_time=$event_data_ar[$i]['end_time'];
                                                $name=$event_data_ar[$i]['name'];?>
                                                <tr style="align-content: center">
                                                    <td class="text-left">{{$i+1}}</td>
                                                    <td class="text-left">{{$event_id}}</td>
                                                    <td class="text-left">{{$cat_event_type}}</td>
                                                    <td class="text-left">{{$date_value}}</td>
